  ...
  return (
    ...
    >
      <div className="h-48 bg-gradient-to-br from-amber-200 to-orange-300 flex items-center justify-center relative overflow-hidden">
        ...
        <Utensils className="h-16 w-16 text-amber-600 z-10 group-hover:animate-float" />
      </div>
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between items-center">
          <span className="text-2xl font-bold text-amber-600 group-hover:text-amber-700 transition-colors duration-200">${item.price}</span>
          <div className="flex items-center space-x-1">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className="h-4 w-4 text-yellow-400 fill-current group-hover:text-yellow-500 transition-colors duration-200" />
            ))}
          </div>
        </div>
        
        <button className="w-full mt-4 bg-amber-600 hover:bg-amber-700 text-white py-2 rounded-lg font-medium transition-all duration-200 transform hover:scale-105 active:scale-95">
          View in AR
        </button>
      </div>
    </div>
