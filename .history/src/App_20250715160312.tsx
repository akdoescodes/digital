...
import { ChefHat, Utensils, Eye, ShoppingCart, Zap } from 'lucide-react';
...
function App() {
  ...
  const [arMode, setArMode] = useState<'camera' | 'webxr' | null>(null);

  useEffect(() => {
    const checkARSupport = async () => {
      try {
        // Check WebXR support first (more accurate)
        ...
        setIsWebXRSupported(webXRSupported);
        setIsARSupported(webXRSupported || hasCamera);
      } catch (error) {
  ...
  const startAR = async (mode: 'camera' | 'webxr') => {
    try {
      setArMode(mode);
      setIsARActive(true);
    } catch (error) {
      console.error('Failed to start AR session:', error);
      alert('AR not available. Please try the other mode.');
    }
  };
